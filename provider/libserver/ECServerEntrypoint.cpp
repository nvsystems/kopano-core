...
ECRESULT kopano_init(std::shared_ptr<ECConfig> cfg, ECLogger *lpAudit,
    bool bHostedKopano, bool bDistributedKopano)
{
	if (!g_bInitLib)
		return KCERR_NOT_INITIALIZED;
	try {
		g_lpSessionManager = new ECSessionManager(std::move(cfg), lpAudit, bHostedKopano, bDistributedKopano);
